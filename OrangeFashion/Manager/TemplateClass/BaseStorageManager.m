...
@property (nonatomic, strong) NSMutableDictionary *allEGCSections;
@property (nonatomic, strong) NSMutableDictionary *allEGCVideos;
...
{
  for (OFSetting *model in self.allEGCSettings.allValues)
...
{
  OFSetting *model = [self getSettingWithKey:key];
...
- (OFSetting*)getSettingWithId:(NSNumber*)key
{
  return [self.allEGCSettings objectForKey:key];
}
- (OFSetting*)addOrUpdateSetting:(NSDictionary*)dict
{
    NSNumber *key = [dict numberForKey:@"id"];
    if ([key integerValue] > 0)
    {
        OFSetting *existingModel = [self getSettingWithId:key];
        if (existingModel != nil) {
            [existingModel updateWithDictionary:dict];
            return existingModel;
        }
    ...
    OFSetting *newModel = [[OFSetting alloc] initWithDictionary:dict];
...
- (BOOL)dumpQuickOfflineCache
{
    NSString *cacheDirectory = [self getOfflineCacheDirectory];
    BOOL success = YES;
    if ([self saveOfflineAllClassObjects:[OFSetting class] directory:cacheDirectory] == NO)     success = NO;
    return success;
}
...
- (BOOL)dumpFullOfflineCache
{
    ...
    BOOL success = YES;
    if ([self saveOfflineAllClassObjects:[OFSetting class] directory:cacheDirectory] == NO)        success = NO;
    if ([self saveOfflineAllClassObjects:[OFProduct class] directory:cacheDirectory] == NO)        success = NO;
//    if ([self saveOfflineAllClassObjects:[EGCVideo class] directory:cacheDirectory] == NO)          success = NO;
//    if ([self saveOfflineAllClassObjects:[EGCSection class] directory:cacheDirectory] == NO)        success = NO;
    return success;
}
...
- (BOOL)restoreOfflineCache
{
    NSString *cacheDirectory = [self getOfflineCacheDirectory];
    BOOL success = YES;
    if ([self restoreOfflineAllClassObjects:[OFSetting class] directory:cacheDirectory] == NO)     success = NO;
    if ([self restoreOfflineAllClassObjects:[OFProduct class] directory:cacheDirectory] == NO)     success = NO;
//    if ([self restoreOfflineAllClassObjects:[EGCVideo class] directory:cacheDirectory] == NO)       success = NO;
//    if ([self restoreOfflineAllClassObjects:[EGCSection class] directory:cacheDirectory] == NO)     success = NO;
    return success;
}
