...
//#define OpenUDIDLog(fmt, ...) DLog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define OpenUDIDLog(fmt, ...) DLog((@"[Line %d] " fmt), __LINE__, ##__VA_ARGS__);
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        DLog(@"------------------");
        DLog(@"From User Defaults");
        DLog(@"OpenUDID creation date: %@", [localDict objectForKey:kOpenUDIDTSKey]);
        DLog(@"OpenUDID value: %@", [localDict objectForKey:kOpenUDIDKey]);
        DLog(@"------------------");
    }
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            DLog(@"------------------");
            DLog(@"From Pasteboard");
            DLog(@"OpenUDID creation date: %@", [dict objectForKey:kOpenUDIDTSKey]);
            DLog(@"OpenUDID value: %@", [dict objectForKey:kOpenUDIDKey]);
            DLog(@"------------------");            
...
+ (NSString*) hashedValue {
  return [[self valueWithKeychain] stringFromMD5];
  //return [[self value] stringFromMD5];
}

+ (NSString*) secretValue {
    NSString *bundleIdentifier = @"com.singtel.auroratest";
    NSString *stringToHash = [NSString stringWithFormat:@"%@%@",[self valueWithKeychain],bundleIdentifier];
    NSString *uniqueSecret = [stringToHash stringFromMD5];
